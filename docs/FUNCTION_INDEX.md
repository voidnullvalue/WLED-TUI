...
| [on_selected_device_changed](#on_selected_device_changed) | `wledtui` | input handling | `on_selected_device_changed(prev_index, next_index)` | update_list_selection, update_topbar_line, current_device_id, fetch_presets… | select_device_delta | jq | shared device-selection transition hook. |
| [select_device_delta](#select_device_delta) | `wledtui` | input handling | `select_device_delta(delta)` | on_selected_device_changed | handle_key, handle_up, handle_down |  | selection clamp + transition helper. |
...
| [parse_discovery_entry](#parse_discovery_entry) | `lib/discover.sh` | discovery | `parse_discovery_entry(entry)` | is_valid_port | discover_devices, process_discover_results | printf | validates/parses `name|host|addr|port` lines. |
